  ...
  return (
    <>
      <header className="bg-[#B5DEF5] w-full shadow-sm sticky top-0 z-50">
        <nav className="max-w-[1200px] mx-auto flex items-center justify-center gap-[10px] p-[15px] ">
          {/* Logo */}
          <Link
            href="/"
          ...
          {/* Desktop Menu */}
          <ul
            className="hidden md:flex items-center  text-sm font-medium mx-14"
            role="menubar"
          >
            {menuItems.map((item: MenuItem, idx: number) => (
              <li
                key={item.label}
                role="none"
                className={item.groups ? "relative group" : ""}
              >
                {item.groups ? (
                  <>
                    <button
                  ...
                  <span
                    className="text-gray-400 cursor-not-allowed font-[700]"
                    role="menuitem"
                  >
                    {item.label}
                  </span>
        ...
        {(isMobileMenuOpen || isClosing) && (
          <>
            ...
            >
              ...
              <ul
                className="flex flex-col gap-2 text-lg font-semibold px-4 pt-10 mt-5"
                role="menu"
              >
                {menuItems.map((item: MenuItem, idx: number) => (
                  <li key={item.label}>
