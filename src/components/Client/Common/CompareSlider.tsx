...
export const CompareSlider: React.FC<CompareSliderProps> = ({ beforeImage, afterImage, onDragStart, onDragEnd }) => {
  ...
  return (
    <div className="h-full" onPointerDown={() => onDragStart?.()} onPointerUp={() => onDragEnd?.()} onPointerLeave={() => onDragEnd?.()}>
      <ReactCompareSlider
        position={position}
        ...
        itemOne={
          <div className="relative w-full h-full">
            ...
            {position > 10 && (
              <div className="absolute top-3 left-3 bg-white/70 text-gray-800 text-xs font-semibold px-3 py-1 rounded-full z-10">Before</div>
            )}
          </div>
        }
        itemTwo={
          <div className="relative w-full h-full">
            ...
            {position < 95 && (
              <div className="absolute top-3 right-3 bg-white/70 text-gray-800 text-xs font-semibold px-3 py-1 rounded-full z-10">After</div>
            )}
          </div>
        }
