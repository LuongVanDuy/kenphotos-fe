...
const BlogList = ({ fetchPublicPosts, postList, postTotal, postLoading }: any) => {
  const [search, setSearch] = useState("");
  const [pageNumber, setPageNumber] = useState(1);
  const [pageSize, setPageSize] = useState(12);
  const [grids, setGrids] = useState<any[][]>([]);

  function handleQuery(search = "", page = 1, itemsPerPage = 12, reset = false) {
    if (reset) setGrids([]);
    fetchPublicPosts({
      search,
      page,
    ...
    <>
      <section className="pb-10 pt-24 md:pt-[180px]">
        <div className="max-w-content px-4 mx-auto">
          ...
          <BlogFilterBar keyword={search} onKeywordChange={(value) => setSearch(value)} onSearch={() => handleQuery(search, 1, pageSize, true)} />
