...
import { Card, Tabs, Button, Avatar, Tag, Descriptions, Space, Divider, Table, message, Skeleton } from "antd";
import { UserOutlined, EditOutlined, DeleteOutlined, ArrowLeftOutlined, LockOutlined, HistoryOutlined, SettingOutlined } from "@ant-design/icons";
...
const UserDetailsPage: React.FC<UserDetailsProps> = ({ params }) => {
  ...
  const tabItems = [
          ...
          {loading ? (
            ...
            <Descriptions bordered column={{ xs: 1, sm: 2, md: 2 }}>
              ...
              <Descriptions.Item label="Role">
                <Tag color={user.role === "admin" ? "red" : user.role === "editor" ? "blue" : "default"}>{user.role.toUpperCase()}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label="Status">
                <Tag color={user.status === "active" ? "green" : user.status === "inactive" ? "red" : "orange"}>{user.status.toUpperCase()}</Tag>
              </Descriptions.Item>
    ...
    },
    {
      ...
      children: <Table dataSource={activityData} columns={activityColumns} rowKey="id" pagination={{ pageSize: 10 }} />,
    },
    {
      key: "settings",
      label: "Settings",
  ...
  return (
    <div>
      <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => router.push("/admin/user/list")} className="mb-4">
        Back to Users
      </Button>
      ...
      <div className="mb-6">
        <div className="flex justify-between items-center">
          <div className="flex items-center">
            {loading ? <Skeleton.Avatar active size={64} shape="circle" /> : <Avatar size={64} icon={<UserOutlined />} src={user?.avatar} />}
            <div className="ml-4">
              {loading ? (
