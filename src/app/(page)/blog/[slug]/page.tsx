...
const BlogDetailPage = ({ fetchPublicPost, postDetail, fetchPublicPosts, postList, postLoading }: any) => {
  ...
  useEffect(() => {
    if (postDetail?.slug && Array.isArray(postDetail.categories) && postDetail.categories.length > 0 && postDetail.categories[0].category?.slug) {
      fetchPublicPosts({
        limitWords: 0,
  ...
  return (
    <div className="max-w-content mx-auto px-4 pt-16 pb-24 md:pt-24 md:pb-36  text-center">
      <h1 className="text-[20px] md:text-[44px]  font-semibold  text-[#0D0D0D]">{postDetail.title}</h1>
      {postDetail.excerpt && <p className="text-base md:text-lg text-gray-600 mt-6 max-w-3xl mx-auto">{postDetail.excerpt}</p>}
      ...
      {postDetail?.thumbnail && (
        <div className="mt-16 w-full aspect-[5/2] relative overflow-hidden rounded-xl shadow">
          <Image src={getImageUrl(postDetail.thumbnail)} alt={postDetail.title || "Blog detail image"} fill className="object-cover" priority />
        </div>
      )}

      <div className="relative flex gap-10 mt-16">
        <div className="hidden lg:flex flex-col gap-6 sticky top-32 h-fit text-gray-400">
          <a href={`https://twitter.com/intent/tweet?url=${url}`} target="_blank" rel="noopener noreferrer" className="hover:text-black transition">
            <svg
